...
void AstarPathFinder::AstarGraphSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  ROS_WARN("[A*] start_pt = %f, %f, %f", start_pt(0), start_pt(1), start_pt(2));
  ROS_WARN("[A*] end_pt = %f, %f, %f", end_pt(0), end_pt(1), end_pt(2));
      ...
      */
      ...
      if (neighborPtr->id == 0) { //discover a new node, which is not in the closed set and open set
        ...
        */
        ...
        neighborPtr->fScore = neighborPtr->gScore + getHeu(neighborPtr, endPtr); // ??? 当时为什么会写个 currentPtr
        neighborPtr->id = 1;
